...
def get_payment_keyboard(product_id: str) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="✅ Я ОПЛАТИЛ", callback_data=f"confirm_{product_id}")
    )
    builder.row(
...
async def show_my_purchases(callback: CallbackQuery):
    ...
    for i, purchase in enumerate(purchases, 1):
        purchases_text += (
            ...
            f"   📎 Ссылка: {purchase['archive_link']}\n\n"
        )
...
async def select_product(callback: CallbackQuery):
    ...
    payment_text = (
        ...
        f"Инструкция:\n"
        f"1. Нажмите кнопку «ОПЛАТИТЬ И ВСТУПИТЬ» ниже.\n"
        f"2. Подтвердите оплату звёздами при вступлении в канал.\n"
        f"3. После оплаты нажмите кнопку «Я ОПЛАТИЛ».\n"
        f"4. Бот пришлёт вам ссылку на скачивание архива!"
    )
